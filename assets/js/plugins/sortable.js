...
(function kvsortableModule(factory) {
  ...
  let dragEl,
    ...
    _autoScroll = _throttle((/** Event */evt, /** Object */options, /** HTMLElement */rootEl) => {
  ...
  KvSortable.prototype = /** @lends KvSortable.prototype */ {
    ...
    _animate (prevRect, target) {
      const ms = this.options.animation;
    ...
    this.each(function () {
      ...
      if (kvsortable) {
        if (options === 'widget') {
          ...
          retVal = kvsortable[options](...[].slice.call(args, 1));
        }				else if (options in kvsortable.options) {
          retVal = kvsortable.option(...args);
        }
      }
    });
