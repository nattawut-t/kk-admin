...
(function (factory) {
  ...
  $h = {
    ...
    cleanArray (arr) {
      ...
      return arr.filter(e => (e !== undefined && e !== null));
    },
    ...
    arrayBuffer2String (buffer) {
      ...
      let array = Array.prototype.slice.apply(new Uint8Array(buffer)),
        out = '',
        i = 0,
        len,
        c,
    ...
    getMimeType (signature, contents, type) {
      switch (signature) {
        case 'ffd8ffe0':
        case 'ffd8ffe1':
        case 'ffd8ffe2':
    ...
    getElement (options, param, value) {
    ...
    cleanMemory ($thumb) {
      const data = $thumb.is('img') ? $thumb.attr('src') : $thumb.find('source').attr('src');
    ...
    cleanZoomCache ($el) {
      let $cache = $el.closest('.kv-zoom-cache-theme');
      if (!$cache.length) {
        $cache = $el.closest('.kv-zoom-cache');
      }
      $cache.remove();
    },
    setOrientation (buffer, callback) {
      let scanner = new DataView(buffer),
        idx = 0,
    ...
    validateOrientation (file, callback) {
    ...
    adjustOrientedImage ($img, isZoom) {
      let offsetContTop,
        offsetTop,
        newTop;
  ...
  FileInput.prototype = {
    ...
    _initTemplateDefaults () {
      ...
      $.each(self.defaults, (key, setting) => {
    ...
    _initPreviewTemplates () {
      ...
      $.each(cfg.previewContentTemplates, (key, value) => {
    ...
    _initPreviewCache () {
      ...
      self.previewCache = {
        data: {},
        init () {
          let content = self.initialPreview;
        ...
        add (content, config, tags, append) {
          let data = self.previewCache.data,
            index;
        ...
        set (content, config, tags, append) {
          let data = self.previewCache.data,
            i,
            chk;
        ...
        unset (index) {
          const chk = self.previewCache.count();
          if (!chk) {
            return;
          }
          if (chk === 1) {
        ...
        footer (i, isDisabled, size) {
          const data = self.previewCache.data || {};
    ...
    _addError (msg) {
      ...
      if (msg && $error.length) {
        ...
        self._handler($error.find('.kv-error-close'), 'click', () => {
          $error.fadeOut('slow');
        });
      }
    },
    _setValidationError (css) {
      const self = this;
    ...
    _showUploadError (msg, params, event) {
      let self = this,
        $error = self.$errorContainer,
    ...
    _parseError (operation, jqXHR, errorThrown, fileName) {
    ...
    _getPreviewIcon (fname) {
      ...
      if (fname && fname.indexOf('.') > -1) {
        ...
        if (self.previewFileExtSettings) {
          $.each(self.previewFileExtSettings, (key, func) => {
    ...
    _parseFilePreviewIcon (content, fname) {
      let self = this,
    ...
    _listenFullScreen (isFullScreen) {
      let self = this,
        $modal = self.$modal,
        $btnFull,
        $btnBord;
    ...
    _listen () {
      ...
      self._handler($(window), 'resize', () => {
        self._listenFullScreen(screen.width === window.innerWidth && screen.height === window.innerHeight);
      });
      fullScreenEvents = 'webkitfullscreenchange mozfullscreenchange fullscreenchange MSFullscreenChange';
      self._handler($(document), fullScreenEvents, () => {
    ...
    _initSortable () {
      ...
      settings = {
        ...
        onSort (e) {
          ...
          self.previewCache.init();
          for (let i = 0; i < self.initialPreviewConfig.length; i++) {
    ...
    _listenModalEvent (event) {
      ...
      $modal.on(`${event}.bs.modal`, e => {
        let $btnFull = $modal.find('.btn-fullscreen'),
    ...
    _resizeZoomDialog (fullScreen) {
      ...
      if ($modal.hasClass('file-zoom-fullscreen')) {
        ...
        } else if (!$btnFull.hasClass('active')) {
          $modal.removeClass('file-zoom-fullscreen');
    ...
    _setZoomContent ($frame, animate) {
      ...
      self._handler($prev, 'click', () => {
        self._zoomSlideShow('prev', pid);
      });
      self._handler($next, 'click', () => {
        self._zoomSlideShow('next', pid);
      });
      self._handler($btnFull, 'click', () => {
        self._resizeZoomDialog(true);
      });
      self._handler($btnBord, 'click', () => {
        self._resizeZoomDialog(false);
      });
      self._handler($btnTogh, 'click', () => {
        let $header = $modal.find('.modal-header'),
          $floatBar = $modal.find('.modal-body .floating-buttons'),
          ht,
          $actions = $header.find('.kv-zoom-actions'),
          resize = function (height) {
            let $body = self.$modal.find('.kv-zoom-body'),
    ...
    _zoomSlideShow (dir, previewId) {
      ...
      self._initZoomButtons();
      self._raise(`filezoom${dir}`, { previewId, modal: self.$modal });
    },
    _initZoomButton () {
      const self = this;
      self.$preview.find('.kv-file-zoom').each(function () {
        const $el = $(this);
        self._handler($el, 'click', () => {
    ...
    _getOutData (jqXHR, responseData, filesData) {
      const self = this;
      jqXHR = jqXHR || {};
    ...
    _initXhr (xhrobj, previewId, fileCount) {
      const self = this;
      if (xhrobj.upload) {
    ...
    _mergeAjaxCallback (funcName, srcFunc, type) {
      let self = this,
    ...
    _ajaxSubmit (fnBefore, fnSuccess, fnComplete, fnError, previewId, index) {
      let self = this,
        settings;
    ...
    _mergeArray (prop, content) {
      let self = this,
        arr1 = $h.cleanArray(self[prop]),
        arr2 = $h.cleanArray(content);
      self[prop] = arr1.concat(arr2);
    },
    _initUploadSuccess (out, $thumb, allFiles) {
      let self = this,
        append,
        data,
        index,
      ...
      if (out.initialPreview !== undefined && out.initialPreview.length > 0) {
        ...
        if ($thumb !== undefined) {
          if (!allFiles) {
            ...
            $thumb.fadeOut('slow', () => {
              const $newThumb = $div.find('.file-preview-frame');
    ...
    _uploadSingle (i, allFiles) {
      ...
      chkComplete = function () {
        ...
        setTimeout(() => {
          const triggerReset = self.getFileStack(true).length === 0;
          if (self.showPreview) {
      ...
      fnSuccess = function (data, textStatus, jqXHR) {
        const pid = self.showPreview && $thumb.attr('id') ? $thumb.attr('id') : previewId;
      ...
      fnError = function (jqXHR, textStatus, errorThrown) {
        let op = self.ajaxOperations.uploadThumb,
    ...
    _uploadBatch () {
      ...
      fnBefore = function (jqXHR) {
        self.lock();
        const outData = self._getOutData(jqXHR);
        if (self.showPreview) {
      ...
      fnSuccess = function (data, textStatus, jqXHR) {
        ...
        let outData = self._getOutData(jqXHR, data),
          key = 0,
      ...
      fnError = function (jqXHR, textStatus, errorThrown) {
        let outData = self._getOutData(jqXHR),
    ...
    _uploadExtraOnly () {
      ...
      fnSuccess = function (data, textStatus, jqXHR) {
        const outData = self._getOutData(jqXHR, data);
        if ($h.isEmpty(data) || $h.isEmpty(data.error)) {
          self._raise('filebatchuploadsuccess', [outData]);
          self._clearFileInput();
          self._initUploadSuccess(data);
      ...
      self.getFrames(' .kv-file-remove').each(function () {
        ...
        self._handler($el, 'click', () => {
          ...
          $frame.fadeOut('slow', () => {
            ...
            self._clearFileInput();
            let filestack = self.getFileStack(true),
    ...
    _initPreviewActions () {
      let self = this,
        ...
        resetProgress = function () {
          const hasFiles = self.isAjaxUpload ? self.previewCache.count() : self.$element.get(0).files.length;
      ...
      $preview.find(btnRemove).each(function () {
        ...
        fnError = function (jqXHR, textStatus, errorThrown) {
          let op = self.ajaxOperations.deleteThumb,
            errMsg = self._parseError(op, jqXHR, errorThrown);
          params.jqXHR = jqXHR;
          params.response = {};
        ...
        self._handler($el, 'click', () => {
          ...
          if (self.ajaxAborted instanceof Promise) {
            self.ajaxAborted.then(result => {
              if (!result) {
                $.ajax(settings);
              }
            });
          } else if (!self.ajaxAborted) {
            $.ajax(settings);
          }
        });
      });
    ...
    _generatePreviewTemplate (cat, data, fname, ftype, previewId, isError, size, frameClass, foot, ind, templ) {
      let self = this,
      ...
      getContent = function (c, d, zoom, frameCss) {
        let id = zoom ? `zoom-${previewId}` : previewId,
        ...
        if (cat === 'object' && !ftype) {
          $.each(self.defaults.fileTypeSettings, (key, func) => {
    ...
    _previewDefault (file, previewId, isDisabled) {
      let self = this,
        $preview = self.$preview;
      if (!self.showPreview) {
        return;
    ...
    _previewFile (i, file, theFile, previewId, data, fileInfo) {
      if (!this.showPreview) {
        return;
      }
      let self = this,
      ...
      if (chkTypes || chkMimes) {
        ...
        self._clearDefaultPreview();
        $preview.append(`\n${content}`);
        const $img = $preview.find(`#${previewId} img`);
        if ($img.length && self.autoOrientImage) {
          $h.validateOrientation(file, value => {
            if (!value) {
              self._validateImage(previewId, caption, ftype, fsize, iData);
              return;
            }
            let $zoomImg = $preview.find(`#zoom-${previewId} img`),
              css = `rotate-${value}`;
            if (value > 4) {
            ...
            self._raise('fileimageoriented', { $img, file });
            self._validateImage(previewId, caption, ftype, fsize, iData);
    ...
    _setThumbAttr (id, caption, size) {
      let self = this,
        $frame = $(`#${id}`);
      if ($frame.length) {
        size = size && size > 0 ? self._getSize(size) : '';
        $frame.data({ caption, size });
      }
    },
    _setInitThumbAttr () {
      let self = this,
    ...
    _readFiles (files) {
      ...
      let self = this,
        ...
        throwError = function (msg, file, previewId, index) {
          let p1 = $.extend(true, {}, self._getOutData({}, {}, files), { id: previewId, index }),
            p2 = { id: previewId, index, file, files },
            $thumb;
      ...
      $.each(files, (key, file) => {
        const func = self.fileTypeSettings.image;
      ...
      readFile = function (i) {
        ...
        let node = ctr + i,
          ...
          processFileLoaded = function () {
            const msg = msgProgress.setTokens({
              index: i + 1,
              files: numFiles,
              percent: 50,
              name: caption,
        ...
        if ($preview.length && FileReader !== undefined) {
          ...
          reader.onprogress = function (data) {
            if (data.lengthComputable) {
              let fact = (data.loaded / data.total) * 100,
    ...
    _setThumbStatus ($thumb, status) {
      const self = this;
      if (!self.showPreview) {
        return;
      }
      let icon = `indicator${status}`,
        msg = `${icon}Title`,
    ...
    _setAsyncUploadStatus (previewId, pct, total) {
      let self = this,
        sum = 0;
      self._setProgress(pct, $(`#${previewId}`).find('.file-thumb-progress'));
      self.uploadStatus[previewId] = pct;
      $.each(self.uploadStatus, (key, value) => {
        sum += value;
      });
    ...
    _getFileIds (skipNull) {
      const self = this;
      return self.fileids.filter(n => (skipNull ? n !== undefined : n !== undefined && n !== null));
    },
    _getFileNames (skipNull) {
      const self = this;
      return self.filenames.filter(n => (skipNull ? n !== undefined : n !== undefined && n !== null));
    },
    _setPreviewError ($thumb, i, val, repeat) {
      const self = this;
      if (i !== undefined) {
    ...
    _refreshUploadButton ($thumb, repeat) {
      let self = this,
    ...
    _checkDimensions (i, chk, $img, $thumb, fname, type, params) {
      let self = this,
        msg,
        dim,
    ...
    _validateImage (previewId, fname, ftype, fsize, iData) {
      let self = this,
        $preview = self.$preview,
        params,
        w1,
    ...
    _getResizedImage (config, counter, numImgs) {
      let self = this,
        img = $(config.img)[0],
    ...
    _renderButton (type) {
      ...
      return tmplt.setTokens({
        type: btnType, css, title, status, icon, label,
      });
    },
    ...
    _renderFileFooter (caption, size, width, isError) {
      let self = this,
        config = self.fileActionSettings,
    ...
    _renderFileActions (showUpl, showDwn, showDel, showZoom, showDrag, disabled, url, key, isInit, dUrl, dFile) {
      ...
      let self = this,
        ...
        config = self.fileActionSettings,
        otherButtons = self.otherActionButtons.setTokens({ dataKey: vKey, key }),
      ...
      if (showDwn) {
        ...
        btnDownload = btnDownload.setTokens({ filename: dFile, key });
      }
      if (showZoom) {
    ...
    _filterDuplicate (file, files, fileIds) {
      let self = this,
    ...
    _change (e) {
      ...
      } else if (!isAjaxUpload || flagSingle) {
        self._resetPreviewThumbs(false);
        if (flagSingle) {
          self.clearStack();
        }
      } else if (isAjaxUpload && ctr === 0 && (!self.previewCache.count() || self.overwriteInitial)) {
    ...
    getFileStack (skipNull) {
      const self = this;
      return self.filestack.filter(n => (skipNull ? n !== undefined : n !== undefined && n !== null));
    },
    getFilesCount () {
      let self = this,
        len = self.isAjaxUpload ? self.getFileStack().length : self.$element.get(0).files.length;
